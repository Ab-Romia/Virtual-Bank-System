...
/**
 * Declares the transfer topics this service consumes and their dead-letter topics
 * so they exist with a single partition and replication factor one, which suits
 * the single-broker development setup. Topic auto-creation is off on the broker,
 * so these beans are how the topics come to exist. Skipped when no broker is
 * configured, for example in tests that exclude Kafka auto-configuration.
 */
